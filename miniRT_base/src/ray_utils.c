...
/*   Updated: 2022/05/13 14:37:45 by rburri           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pixel_colors(t_data d, t_ray *ray, float specular)
{
	...
	b = fmin(ray->sf_color[2], ray->sf_color[2] * d.s->light->ratio * L_INT \
		* fmax(0, dot_p(&tmp, &ray->nrm)) / (norm_sq(&ray->pos)));
	r = fmax(r + d.s->light->ratio * L_INT * specular * 0.15, ray->sf_color[0] \
		* L_INT * d.s->amb->light * 0.0003);
	g = fmax(g + d.s->light->ratio * L_INT * specular * 0.15, ray->sf_color[1] \
		* L_INT * d.s->amb->light * 0.0003);
	b = fmax(b + d.s->light->ratio * L_INT * specular * 0.15, ray->sf_color[2] \
		* L_INT * d.s->amb->light * 0.0003);
